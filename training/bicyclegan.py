...
from colorize_microservice.settings import main_opt
...
parser.add_argument("--train_set_folder", type=str, default="", help="path to dataset")
...
parser.add_argument("--latent_dim", type=int, default=main_opt.latent_dim, help="number of latent codes")
...
input_shape = (main_opt.channels, main_opt.img_y, main_opt.img_x)
...
if opt.epoch != 0:
    ...
    generator.load_state_dict(torch.load("saved_models/%s/generator_%d.pth" % (opt.dataset_name, opt.epoch-1)))
    encoder.load_state_dict(torch.load("saved_models/%s/encoder_%d.pth" % (opt.dataset_name, opt.epoch-1)))
    D_VAE.load_state_dict(torch.load("saved_models/%s/D_VAE_%d.pth" % (opt.dataset_name, opt.epoch-1)))
    D_LR.load_state_dict(torch.load("saved_models/%s/D_LR_%d.pth" % (opt.dataset_name, opt.epoch-1)))
else:
    # Initialize weights
...
dataloader = DataLoader(
    ImageDataset(opt.train_set_folder, input_shape),
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=opt.n_cpu,
)
val_dataloader = DataLoader(
    ImageDataset(opt.train_set_folder, input_shape, mode="val"),
    batch_size=8,
    shuffle=True,
    num_workers=1,
)
